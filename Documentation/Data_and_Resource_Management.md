...
- 필드 예시:
  - `EnemyCount`, `SpawnIntervalSeconds`, `SpawnRadius`, `SpawnCenterWorldOffset`
  - `PooledEnemyPrefab`
- 규칙:
  - `EncounterDirector`는 실행기(Executor)로 유지하고 스폰 파라미터는 SO에서만 관리
  - 퀘스트·세이브와의 연동은 `QuestService`·`WorldStateService`에서 담당
