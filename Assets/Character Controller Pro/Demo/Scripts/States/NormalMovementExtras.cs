...
namespace Lightbug.CharacterControllerPro.Demo
{
    ...
    public class VerticalMovementParameters
    {
        ...
        [Tooltip("When a character touches the ground, if the jump started action elapsed time is less than this value, a " +
            "grounded jump will be performed. This is commonly know as jump input buffering. It is called \"pre\" grounded time " +
            "because it allows to trigger a grounded jump before actually touching the ground.")]
        [Min(0f)]
        public float preGroundedJumpTime = 0.2f;

        [Tooltip("When a character goes \"not grounded\", this value acts as a (usually) small time window during which a grounded jump " +
            "is still available even when there is no contact with the ground. This is commonly know as \"coyote time\". " +
            "It is called \"post\" grounded time because it allows to trigger a grounded jump after leaving the grounded state")]
        [Min(0f)]
        ...
        [Tooltip("Even when the character is not able to jump on unstable ground, a \"pre\" jump could be started " +
            "(e.g. button pressed) and then (e.g. 10 milliseconds later) executed after reaching stable ground. " +
            "\n\nTurn this boolean to true if you don't want to allow this behaviour.")]
        public bool unstableGroundedResetsPreJump = false;
