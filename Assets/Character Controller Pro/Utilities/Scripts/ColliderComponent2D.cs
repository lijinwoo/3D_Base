...
namespace Lightbug.Utilities
{
    ...
    public abstract class ColliderComponent2D : ColliderComponent
    {
        ...
        protected abstract int InternalOverlapBody(Vector3 position, Quaternion rotation, Collider2D[] unfilteredResults);

        public sealed override int OverlapBody(Vector3 position, Quaternion rotation)
        {
            var overlaps = InternalOverlapBody(
                position,
                rotation,
                UnfilteredOverlaps);

            return FilterValidOverlaps(
                overlaps,
                UnfilteredOverlaps,
                FilteredOverlaps,
                null);
